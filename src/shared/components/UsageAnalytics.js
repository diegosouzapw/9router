...
/** Summary stat card */
function StatCard({ icon, label, value, subValue, color = "text-text-main" }) {
  return (
...
function ActivityHeatmap({ activityMap }) {
  ...
  const weeks = useMemo(() => {
    const w = [];
    let current = [];
    // Pad start so first column starts at Sunday
...
/** Daily Trend Bar Chart — simple CSS bars */
...
/** Usage Detail side-by-side card */
function UsageDetail({ summary }) {
  const items = [
