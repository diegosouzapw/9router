...
export default function ProxyLogger() {
  ...
  const toggleColumn = useCallback((key) => {
    setVisibleColumns(prev => {
      ...
      try { localStorage.setItem("proxyLoggerVisibleColumns", JSON.stringify(next)); } catch {}
      return next;
    });
  }, []);
  ...
  const sortedLogs = useMemo(() => {
    ...
    arr.sort((a, b) => {
      switch (sortBy) {
        ...
        case "latency_desc": return (b.latencyMs || 0) - (a.latencyMs || 0);
        case "latency_asc": return (a.latencyMs || 0) - (b.latencyMs || 0);
        case "newest":
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        >
          ...
          {uniqueTypes.map(t => (
            <option key={t} value={t}>{(TYPE_COLORS[t]?.label || t).toUpperCase()}</option>
          ))}
        </select>
        ...
        >
          ...
          {uniqueLevels.map(l => (
            <option key={l} value={l}>{LEVEL_COLORS[l]?.label || l}</option>
          ))}
        </select>
        ...
        <div className="flex items-center gap-2 text-xs text-text-muted">
          <span className="px-2 py-1 rounded bg-bg-subtle border border-border font-mono">{totalCount} total</span>
          <span className="px-2 py-1 rounded bg-emerald-500/10 text-emerald-400 font-mono">{okCount} OK</span>
          {errorCount > 0 && (
            <span className="px-2 py-1 rounded bg-red-500/10 text-red-400 font-mono">{errorCount} ERR</span>
          )}
          {timeoutCount > 0 && (
            <span className="px-2 py-1 rounded bg-amber-500/10 text-amber-400 font-mono">{timeoutCount} TMO</span>
          )}
          {directCount > 0 && (
            <span className="px-2 py-1 rounded bg-gray-500/10 text-gray-400 font-mono">{directCount} direct</span>
          )}
        </div>
          ...
          <button
            ...
            className={`flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-medium border transition-all ${
              activeFilter === f.key
                ...
                : f.key === "timeout" ? "bg-amber-500/20 text-amber-400 border-amber-500/40"
        ...
        <div className="p-0 overflow-x-auto max-h-[calc(100vh-320px)] overflow-y-auto">
          {loading && logs.length === 0 ? (
            ...
            <div className="p-8 text-center text-text-muted">
              <span className="material-symbols-outlined text-[48px] mb-2 block opacity-40">vpn_lock</span>
            ...
            <table className="w-full text-left border-collapse text-xs">
              <thead className="sticky top-0 z-10" style={{ backgroundColor: "var(--bg-primary, #0f1117)" }}>
                <tr className="border-b border-border" style={{ backgroundColor: "var(--bg-primary, #0f1117)" }}>
                  ...
                  {visibleColumns.proxy && <th className="px-3 py-2.5 font-semibold text-text-muted uppercase tracking-wider text-[10px]">Proxy</th>}
                  {visibleColumns.type && <th className="px-3 py-2.5 font-semibold text-text-muted uppercase tracking-wider text-[10px]">Type</th>}
                  {visibleColumns.level && <th className="px-3 py-2.5 font-semibold text-text-muted uppercase tracking-wider text-[10px]">Level</th>}
                  ...
                  {visibleColumns.target && <th className="px-3 py-2.5 font-semibold text-text-muted uppercase tracking-wider text-[10px]">Target</th>}
                  {visibleColumns.latency && <th className="px-3 py-2.5 font-semibold text-text-muted uppercase tracking-wider text-[10px] text-right">Latency</th>}
                  {visibleColumns.ip && <th className="px-3 py-2.5 font-semibold text-text-muted uppercase tracking-wider text-[10px]">Public IP</th>}
              ...
              <tbody className="divide-y divide-border/30">
                {sortedLogs.map((log) => {
                  ...
                  const typeColor = TYPE_COLORS[log.proxy?.type] || { bg: "#6B7280", text: "#fff", label: log.proxy?.type || "-" };
                  ...
                  return (
                    ...
                    >
                      ...
                      {visibleColumns.target && (
                        <td className="px-3 py-2 text-text-muted truncate max-w-[200px] font-mono text-[10px]" title={log.targetUrl}>
                          {truncateUrl(log.targetUrl)}
                        </td>
                      )}
                      {visibleColumns.latency && (
                        <td className="px-3 py-2 text-right text-text-muted font-mono">{formatLatency(log.latencyMs)}</td>
                      )}
                      {visibleColumns.ip && (
                        <td className="px-3 py-2 font-mono text-[11px] text-emerald-400">{log.publicIp || "—"}</td>
                      )}
                      {visibleColumns.time && (
      ...
      >
        ...
        <div className="p-6 flex flex-col gap-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4 p-4 bg-bg-subtle rounded-xl border border-border">
            ...
            <div>
              <div className="text-[10px] text-text-muted uppercase tracking-wider mb-1">Public IP</div>
              <div className="text-sm font-medium font-mono text-emerald-400">{log.publicIp || "—"}</div>
            </div>
            <div>
            ...
            <div className="col-span-2">
              <div className="text-[10px] text-text-muted uppercase tracking-wider mb-1">Target URL</div>
              <div className="text-sm font-medium font-mono text-text-muted break-all">{log.targetUrl || "—"}</div>
            </div>
          </div>
          ...
          {log.proxy && (
            <div className="p-4 rounded-xl bg-bg-subtle border border-border">
              <div className="text-[10px] text-text-muted uppercase tracking-wider mb-2 font-bold">Proxy Configuration</div>
