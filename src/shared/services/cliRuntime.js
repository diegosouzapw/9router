...
const getRuntimeMode = () => {
  const mode = String(process.env.CLI_MODE || "auto").trim().toLowerCase();
...
const locateCommand = async (command, env) => {
  ...
  const located = await runProcess("sh", ["-c", 'command -v -- "$1"', "sh", command], { env, timeoutMs: 3000 });
  if (!located.ok || !located.stdout) {
    return { installed: false, commandPath: null, reason: "not_found" };
  }
  const first = located.stdout.split(/\r?\n/).map((line) => line.trim()).find(Boolean) || null;
...
export const isCliConfigWriteAllowed = () => parseBoolean(process.env.CLI_ALLOW_CONFIG_WRITES, true);
...
export const getCliConfigHome = () => String(process.env.CLI_CONFIG_HOME || "").trim() || os.homedir();
