...
export default function TestBenchMode() {
  ...
  useEffect(() => {
    const fetchProviders = async () => {
      try {
        const [connRes, nodesRes] = await Promise.all([fetch("/api/providers"), fetch("/api/provider-nodes")]);
  ...
  const runScenario = async (scenario) => {
    ...
    try {
      ...
      if (!body) {
        setResults((prev) => ({ ...prev, [scenario.id]: { status: "error", error: "No template for this format", latency: 0 } }));
        return;
      }
      ...
      if (!translateData.success) {
        setResults((prev) => ({ ...prev, [scenario.id]: { status: "error", error: `Translation failed: ${translateData.error}`, latency: Date.now() - start } }));
        return;
      }
      ...
      <Card>
        <div className="p-4 flex flex-col sm:flex-row items-end gap-4">
          <div className="flex-1 w-full">
            <label className="block text-xs font-medium text-text-muted mb-1.5 uppercase tracking-wider">Source Format</label>
            <Select
              value={sourceFormat}
              onChange={(e) => { setSourceFormat(e.target.value); setResults({}); }}
              options={[
          ...
          <div className="flex items-center justify-center px-2">
            <span className="material-symbols-outlined text-[22px] text-text-muted">arrow_forward</span>
          </div>
          <div className="flex-1 w-full">
            <label className="block text-xs font-medium text-text-muted mb-1.5 uppercase tracking-wider">Target Provider</label>
            <Select value={provider} onChange={(e) => { setProvider(e.target.value); setResults({}); }} options={providerOptions} />
          </div>
          <Button icon="play_arrow" onClick={handleRunAll} loading={runningAll} disabled={runningAll}>
            Run All Tests
          </Button>
        </div>
      </Card>
          ...
          <div className="p-4">
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-3">
                ...
                <Badge variant={compatibility >= 80 ? "success" : compatibility >= 50 ? "warning" : "error"} size="lg">
                  {compatibility}%
                </Badge>
              </div>
              ...
              <div className="p-4 space-y-3">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <div className={`flex items-center justify-center w-9 h-9 rounded-lg ${result?.status === "pass"
                ...
                {result && result.status !== "running" && (
                  <div className={`rounded-lg p-2 text-xs ${result.status === "pass" ? "bg-green-500/5 text-green-600 dark:text-green-400" : "bg-red-500/5 text-red-600 dark:text-red-400"}`}>
                    {result.status === "pass" ? (
                      <div className="flex items-center justify-between">
                        <span>✅ Passed</span>
                        <span className="text-text-muted">{result.latency}ms • {result.chunks} chunks</span>
                      </div>
                    ) : (
                      <div>
                        <p>❌ {result.error}</p>
