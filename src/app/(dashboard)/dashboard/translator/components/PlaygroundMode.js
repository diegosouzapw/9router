...
import { EXAMPLE_TEMPLATES, FORMAT_META, FORMAT_OPTIONS } from "../data/exampleTemplates";
    ...
    try {
      ...
      const res = await fetch("/api/translator/translate", {
        ...
        body: JSON.stringify({
          ...
          sourceFormat,
          targetFormat,
          provider: targetFormat === "claude" ? "anthropic" : targetFormat === "gemini" ? "google" : "openai",
          body: parsed,
        }),
      });
      const data = await res.json();
        ...
        <div className="p-4 flex flex-col sm:flex-row items-center gap-4">
          ...
          <div className="flex-1 w-full">
            ...
            <div className="flex items-center gap-2">
              ...
              <Select
                value={sourceFormat}
                onChange={(e) => { setSourceFormat(e.target.value); setDetectedFormat(null); }}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          <div className="p-4 space-y-3">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-1">
                ...
                <button
                  onClick={() => { setInputContent(""); setOutputContent(""); setDetectedFormat(null); setActiveTemplate(null); }}
                  className="p-1.5 rounded hover:bg-black/5 dark:hover:bg-white/5 text-text-muted hover:text-text-main transition-colors"
                  title="Clear"
                >
        ...
        <Card>
          <div className="p-4 space-y-3">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <span className="material-symbols-outlined text-[18px] text-text-muted">output</span>
        ...
        <div className="p-4 space-y-3">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-2">
            {EXAMPLE_TEMPLATES.map((template) => (
              <button
                ...
                `}
              >
                <span className={`material-symbols-outlined text-[22px] ${activeTemplate === template.id ? "text-primary" : "text-text-muted group-hover:text-primary"} transition-colors`}>
                  {template.icon}
                </span>
          ...
          {activeTemplate && (
            <div className="flex items-center gap-2 text-xs text-text-muted">
              ...
              Template loads the request in <strong className="text-text-main">{FORMAT_META[sourceFormat]?.label || sourceFormat}</strong> format. 
              Change Source Format to load in a different format.
            </div>
          )}
        </div>
      </Card>
