  ...
  useEffect(() => {
    const fetchProviders = async () => {
      try {
        ...
        const options = [...activeProviders].map((pid) => {
          const info = AI_PROVIDERS[pid];
          const node = nodeMap.get(pid);
          let label = info?.name || node?.name || pid;
          if (!info && pid.startsWith(OPENAI_COMPATIBLE_PREFIX)) label = node?.name || "OpenAI Compatible";
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      steps.push({
        ...
        content: JSON.stringify({ detectedFormat, clientFormat, match: detectedFormat === clientFormat }, null, 2),
        status: "done",
      });
      ...
      const toOpenaiRes = await fetch("/api/translator/translate", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ step: "direct", sourceFormat: detectedFormat, targetFormat: "openai", body: clientRequest }),
      });
      ...
      if (!sendRes.ok) {
        ...
        setChatHistory((prev) => [...prev, { role: "assistant", content: `Error: ${errData.error || "Request failed"}` }]);
      } else {
        ...
        steps.push({
          ...
          name: "Provider Response",
          format: targetFmt,
          content: fullResponse.slice(0, 5000) + (fullResponse.length > 5000 ? "\n... (truncated)" : ""),
          status: "done",
        });

        // Extract assistant text from SSE
        const assistantText = extractAssistantText(fullResponse);
        setChatHistory((prev) => [...prev, { role: "assistant", content: assistantText || "(No text extracted)" }]);
      }
    } catch (err) {
      steps.push({
        id: steps.length + 1,
        ...
        <Card>
          <div className="p-4 flex flex-col sm:flex-row gap-3">
            <div className="flex-1">
              <label className="block text-xs font-medium text-text-muted mb-1 uppercase tracking-wider">Client Format</label>
              <Select value={clientFormat} onChange={(e) => setClientFormat(e.target.value)} options={FORMAT_OPTIONS.filter((o) => ["openai", "claude", "gemini", "openai-responses"].includes(o.value))} />
            </div>
            <div className="flex-1">
              <label className="block text-xs font-medium text-text-muted mb-1 uppercase tracking-wider">Provider</label>
              <Select value={provider} onChange={(e) => setProvider(e.target.value)} options={providerOptions} />
            </div>
          </div>
        </Card>
          ...
          <div className="p-3 border-t border-border">
            <div className="flex gap-2">
              <input
                ...
                disabled={sending}
              />
              <Button icon="send" onClick={handleSend} loading={sending} disabled={!message.trim() || sending}>
                Send
              </Button>
            </div>
          </div>
        ...
        {!pipeline ? (
          <Card>
            <div className="p-8 flex flex-col items-center justify-center text-text-muted">
              <span className="material-symbols-outlined text-[48px] mb-3 opacity-30">account_tree</span>
          ...
          <div className="space-y-2">
            {pipeline.map((step, i) => {
              const meta = FORMAT_META[step.format] || { label: step.format, color: "gray", icon: "code" };
              ...
              return (
                <div key={step.id}>
                  ...
                  <Card className={step.status === "error" ? "border-red-500/30" : isExpanded ? "border-primary/30" : ""}>
                    <button
                      onClick={() => setExpandedStep(isExpanded ? null : step.id)}
                      className="w-full p-3 flex items-center gap-3 text-left"
                    >
                      {/* Step number */}
                      <div className={`flex items-center justify-center w-7 h-7 rounded-full text-xs font-bold ${step.status === "error"
                      ...
                      {/* Format badge */}
                      <Badge
                        variant={step.status === "error" ? "error" : "default"}
                        size="sm"
                      >
                        {meta.label}
                      </Badge>
