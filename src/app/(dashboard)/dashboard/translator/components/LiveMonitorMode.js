      ...
      <Card>
        <div className="p-3 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <span className={`material-symbols-outlined text-[18px] ${autoRefresh ? "text-green-500 animate-pulse" : "text-text-muted"}`}>
        ...
        <div className="p-4">
          ...
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-12 text-text-muted">
              <span className="material-symbols-outlined text-[48px] mb-3 opacity-30">monitoring</span>
              <p className="text-sm font-medium mb-1">No translations yet</p>
              <p className="text-xs">Translations will appear here as requests flow through the proxy.</p>
              <p className="text-xs mt-2">
                Make API calls to your 9Router endpoints to see live translation data.
              </p>
            </div>
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {events.map((event, i) => {
                    const srcMeta = FORMAT_META[event.sourceFormat] || { label: event.sourceFormat || "?", color: "gray" };
                    const tgtMeta = FORMAT_META[event.targetFormat] || { label: event.targetFormat || "?", color: "gray" };

                    return (
                      <tr key={event.id || i} className="border-b border-border/50 hover:bg-bg-subtle/50 transition-colors">
                        <td className="py-2 pr-4 text-xs text-text-muted whitespace-nowrap">
                          {event.timestamp ? new Date(event.timestamp).toLocaleTimeString() : "—"}
                        </td>
                        <td className="py-2 pr-4">
                          <Badge variant="default" size="sm">{srcMeta.label}</Badge>
                        </td>
                        <td className="py-2 pr-4 text-text-muted">
                          <span className="material-symbols-outlined text-[14px]">arrow_forward</span>
                        </td>
                        <td className="py-2 pr-4">
                          <Badge variant="primary" size="sm">{tgtMeta.label}</Badge>
                        </td>
                        <td className="py-2 pr-4 text-xs font-mono text-text-muted">{event.model || "—"}</td>
                        <td className="py-2 pr-4">
                          {event.status === "success" ? (
                            <Badge variant="success" size="sm" dot>OK</Badge>
                          ) : (
                            <Badge variant="error" size="sm" dot>{event.statusCode || "ERR"}</Badge>
                          )}
                        </td>
                        <td className="py-2 text-right text-xs text-text-muted">{event.latency ? `${event.latency}ms` : "—"}</td>
                      </tr>
                    );
                  })}
                </tbody>
