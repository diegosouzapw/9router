  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {combos.length === 0 ? (
        <Card>
          <div className="text-center py-12">
            ...
            <p className="text-sm text-text-muted mb-4">Create model combos with weighted routing and fallback support</p>
            <Button icon="add" onClick={() => setShowCreateModal(true)}>
              Create Combo
            </Button>
          </div>
      ...
      {testResults && (
        <Modal
          isOpen={!!testResults}
          onClose={() => { setTestResults(null); setTestingCombo(null); }}
                ...
                models.slice(0, 3).map((entry, index) => {
                  ...
                  return (
                    <code key={index} className="text-[10px] font-mono bg-black/5 dark:bg-white/5 px-1.5 py-0.5 rounded text-text-muted">
                      {model}{strategy === "weighted" && weight > 0 ? ` (${weight}%)` : ""}
                    </code>
                  );
                })
              )}
            ...
            {metrics && (
              <div className="flex items-center gap-3 mt-1">
                <span className="text-[10px] text-text-muted">
                  <span className="text-emerald-500">{metrics.totalSuccesses}</span>/{metrics.totalRequests} reqs
                </span>
                <span className="text-[10px] text-text-muted">
    ...
    <div className="flex flex-col gap-2">
      {results.resolvedBy && (
        <div className="flex items-center gap-2 text-sm">
          ...
          <span>Resolved by: <code className="text-xs bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 px-1.5 py-0.5 rounded">{results.resolvedBy}</code></span>
        </div>
      )}
      {results.results?.map((r, i) => (
        <div key={i} className="flex items-center gap-2 text-xs px-2 py-1.5 rounded bg-black/[0.02] dark:bg-white/[0.02]">
          <span className={`material-symbols-outlined text-[14px] ${
            r.status === "ok" ? "text-emerald-500" :
            r.status === "skipped" ? "text-text-muted" :
            "text-red-500"
          }`}>
...
function ComboFormModal({ isOpen, combo, onClose, onSave, activeProviders }) {
  ...
  const handleWeightChange = (index, weight) => {
    ...
    newModels[index] = { ...newModels[index], weight: Math.max(0, Math.min(100, Number(weight) || 0)) };
    setModels(newModels);
  };
        ...
        <div className="flex flex-col gap-3">
          ...
          <div>
            ...
            <div className="flex gap-1 p-0.5 bg-black/5 dark:bg-white/5 rounded-lg">
              ...
              >
                <span className="material-symbols-outlined text-[14px] align-middle mr-1">sort</span>
                Priority
              </button>
              <button
            ...
            {models.length === 0 ? (
              ...
              <div className="flex flex-col gap-1 max-h-[240px] overflow-y-auto">
                {models.map((entry, index) => (
                  ...
                  >
                    {/* Drag handle */}
                    <span className="material-symbols-outlined text-[14px] text-text-muted/40 cursor-grab shrink-0">drag_indicator</span>
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  ...
                  <input
                    ...
                    placeholder="1"
                    onChange={(e) => setConfig({ ...config, maxRetries: e.target.value ? Number(e.target.value) : undefined })}
                    className="w-full text-xs py-1.5 px-2 rounded border border-black/10 dark:border-white/10 bg-transparent focus:border-primary focus:outline-none"
                  />
                </div>
                <div>
                  <label className="text-[10px] text-text-muted mb-0.5 block">Retry Delay (ms)</label>
                  <input
                    type="number"
                    min="0"
                    max="60000"
                    ...
                    placeholder="2000"
                    onChange={(e) => setConfig({ ...config, retryDelayMs: e.target.value ? Number(e.target.value) : undefined })}
                    className="w-full text-xs py-1.5 px-2 rounded border border-black/10 dark:border-white/10 bg-transparent focus:border-primary focus:outline-none"
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    placeholder="120000"
                    onChange={(e) => setConfig({ ...config, timeoutMs: e.target.value ? Number(e.target.value) : undefined })}
                    className="w-full text-xs py-1.5 px-2 rounded border border-black/10 dark:border-white/10 bg-transparent focus:border-primary focus:outline-none"
                  />
                </div>
                <div className="flex items-center gap-2">
