...
export default function KiloToolCard({
  ...
  const getConfigStatus = () => {
    ...
    if (!kiloStatus.has9Router) return "not_configured";
    return "configured";
  };
  ...
  return (
    <Card padding="sm" className={`overflow-hidden ${(statusStyle || (configStatus && { configured: 'border-l-2 border-l-green-500', not_configured: 'border-l-2 border-l-yellow-500' }[configStatus]))?.border || statusStyle?.border || ''}`}>
      <div className="flex items-center justify-between hover:cursor-pointer" onClick={onToggle}>
        <div className="flex items-center gap-3">
          <div className="size-8 rounded-lg flex items-center justify-center shrink-0">
            {tool.image ? (
              <Image src={tool.image} alt={tool.name} width={32} height={32}
                className="size-8 object-contain rounded-lg" sizes="32px"
                onError={(e) => { e.target.style.display = "none"; }}
              />
            ) : (
              <span className="material-symbols-outlined text-xl" style={{ color: tool.color }}>terminal</span>
            )}
          </div>
          ...
          {kiloStatus && !checkingKilo && (
            <div className="flex flex-col gap-4">
              ...
              <div className="flex items-start gap-3 p-3 rounded-lg border bg-bg-secondary/50 border-border">
                <span className={`material-symbols-outlined text-lg ${cliReady ? "text-green-500" : "text-yellow-500"}`}>{cliReady ? "check_circle" : "warning"}</span>
                <div className="flex flex-col gap-1">
                  ...
                  {kiloStatus.commandPath && (
                    <p className="text-xs text-text-muted">Binary: <code className="px-1 py-0.5 rounded bg-black/5 dark:bg-white/10">{kiloStatus.commandPath}</code></p>
                  )}
                  {kiloStatus.authPath && (
                    <p className="text-xs text-text-muted">Auth: <code className="px-1 py-0.5 rounded bg-black/5 dark:bg-white/10">{kiloStatus.authPath}</code></p>
                  )}
                </div>
              </div>
                ...
                <>
                  ...
                  {configStatus === "configured" && (
                    <div className="flex items-start gap-3 p-3 bg-green-500/10 border border-green-500/30 rounded-lg">
                      ...
                      <div className="flex flex-col gap-1">
                        <p className="text-sm text-green-700 dark:text-green-300">9Router is configured as OpenAI-compatible provider</p>
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      />
                      <Button variant="outline" size="sm" onClick={() => setModalOpen(true)} disabled={!hasActiveProviders}>
                        Select
                      </Button>
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    {apiKeys && apiKeys.length > 0 ? (
                      ...
                      <p className="text-sm text-text-muted">{cloudEnabled ? "No API keys available" : "Using default: sk_9router"}</p>
                    )}
                  </div>
                  ...
                  {/* Message */}
                  {message && (
                    <div className={`flex items-center gap-2 px-3 py-2 rounded-lg text-sm ${message.type === "success" ? "bg-green-500/10 text-green-600" : "bg-red-500/10 text-red-600"}`}>
                  ...
                  <div className="border-t border-border pt-3 mt-1">
                    ...
                    >
                      <span className={`material-symbols-outlined text-[16px] transition-transform ${showBackups ? "rotate-90" : ""}`}>chevron_right</span>
                    ...
                    {showBackups && backups.length > 0 && (
                      <div className="mt-2 flex flex-col gap-1.5 pl-6">
                        {backups.map((b) => (
                          <div key={b.id} className="flex items-center justify-between gap-2 p-2 rounded bg-bg-secondary/50 text-xs">
                            <div className="flex flex-col">
                              <span className="font-medium">{b.originalFile}</span>
                              <span className="text-text-muted">{new Date(b.createdAt).toLocaleString()}</span>
                            </div>
      ...
      <ManualConfigModal
        ...
        currentConfig={{ model: selectedModel, apiKey: selectedApiKey, baseUrl: customBaseUrl || baseUrl }}
