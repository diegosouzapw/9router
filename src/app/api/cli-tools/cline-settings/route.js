...
// Check if 9Router is configured as OpenAI-compatible provider
const has9RouterConfig = (globalState) => {
  if (!globalState) return false;
  const isOpenAi = globalState.actModeApiProvider === "openai" || 
...
// POST - Configure Cline to use 9Router as OpenAI-compatible provider
export async function POST(request) {
  try {
    ...
    } catch { /* No existing secrets */ }

    secrets.openAiApiKey = apiKey || "sk_9router";
...
// DELETE - Remove 9Router OpenAI-compatible provider config
