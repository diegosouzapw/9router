...
async function saveContinueConfig({ baseUrl, apiKey, model }) {
  ...
  // Check if 9Router entry already exists and update it, or add new
  const existingIdx = models.findIndex(
    (m) => m.apiBase && (m.apiBase.includes("localhost:20128") || m.apiBase.includes("9router") || m.title === model)
  );
