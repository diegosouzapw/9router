...
export async function computeAnalytics(history, range = "30d", connectionMap = {}) {
  ...
  for (const entry of entries) {
    ...
    // Daily trend
    if (!dailyMap[dateKey]) {
      dailyMap[dateKey] = { date: dateKey, requests: 0, promptTokens: 0, completionTokens: 0, cost: 0 };
    }
    ...
    if (!byModelMap[modelShort]) {
      byModelMap[modelShort] = { model: modelShort, provider: entry.provider, requests: 0, promptTokens: 0, completionTokens: 0, totalTokens: 0, cost: 0 };
    }
    ...
    if (!byProviderMap[prov]) {
      byProviderMap[prov] = { provider: prov, requests: 0, promptTokens: 0, completionTokens: 0, totalTokens: 0, cost: 0 };
    }
