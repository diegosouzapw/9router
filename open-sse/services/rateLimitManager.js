...
function getLimiter(provider, connectionId, model = null) {
  ...
  if (!limiters.has(key)) {
    ...
    limiter.on("queued", () => {
      ...
      if (counts.QUEUED > 0) {
        console.log(`⏳ [RATE-LIMIT] ${key} — ${counts.QUEUED} request(s) queued, ${counts.RUNNING} running`);
      }
    });
...
export function updateFromHeaders(provider, connectionId, headers, status, model = null) {
  ...
  const limiter = getLimiter(provider, connectionId, null);
  const headerMap = provider === "claude" || provider === "anthropic"
    ? ANTHROPIC_HEADERS
    : STANDARD_HEADERS;
  ...
  if (status === 429) {
    ...
    console.log(`🚫 [RATE-LIMIT] ${provider}:${connectionId.slice(0, 8)} — 429 received, pausing for ${Math.ceil(retryAfterMs / 1000)}s`);
  ...
  if (overLimit === "yes") {
    console.log(`⚠️ [RATE-LIMIT] ${provider}:${connectionId.slice(0, 8)} — near capacity, slowing down`);
    limiter.updateSettings({
  ...
  if (!isNaN(limit) && limit > 0) {
    ...
    if (!isNaN(remaining)) {
      if (remaining < limit * 0.1) {
        ...
        console.log(`⚠️ [RATE-LIMIT] ${provider}:${connectionId.slice(0, 8)} — ${remaining}/${limit} remaining, throttling`);
